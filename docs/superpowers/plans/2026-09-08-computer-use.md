...
### Task 22: Claude-style HUD and glow — animated spark logo, human action captions, key-cap hint, motion design; orchestrator writes the task title

User feedback (2026-09-08): "design the glow and the window Claude shows in Claude's style — nice animations, the dancing Claude spark like in Claude Code; the agent that launches computer use should write the task nicely in that top window."

**Files:**
- Modify: `internal/overlay/hud.go` (+`hud_test.go`), new `internal/overlay/spark.go` (+test), `internal/overlay/overlay_windows.go` (animation state: appear/disappear, action crossfade, spark phase), `internal/overlay/border.go` (optional shimmer), `internal/overlay/overlay.go` (`Config.Shimmer bool`), `internal/config` (`border_shimmer` bool default true), `internal/server/session.go` (action summaries become `verb-key|detail`, e.g. `click|640,412`), `skills/computer-use/SKILL.md`, `agents/operator.md`, `cmd/cu/cmd_demo.go` (demo cycles through verbs), README (one paragraph + updated screenshot/GIF note).

**Design (pure rendering in Go, all windows via UpdateLayeredWindow as today):**
- **Palette (Claude brand):** accent terracotta `#D97757` (already `accent`), pill background warm dark `#1F1E1B` at α 0.92, hairline `#D97757` at α 0.35, title cream `#F4F3EE`, subtitle warm gray `#B1ADA1`, paused: icon/hairline gray `#8A8780`, background `#2A2926`.
- **Spark logo (`spark.go`):** `renderSpark(img, cx, cy, r float64, phase float64, c color.RGBA)` draws the Claude starburst: 12 rays from the center, ray length `r·(0.78 + 0.22·sin(3·angle + phase))` (irregular like the real mark), ray width tapering from `0.18r` at the center to `0.05r` at the tip, anti-aliased with `blendPixel`; the whole mark rotates slowly (`phase` advances 2π per 8 s) and breathes (scale 0.92–1.08 with the border breathing). Paused: gray, static.
- **Layout:** `[spark 22px] [title 15px medium] … [Esc][Esc] hint` on line 1 → no: two lines as now, but line 1 = spark + title (task caption), line 2 = action caption + right-aligned key-caps: two small rounded rects (`#3A3835` bg, `#F4F3EE` text "Esc") followed by the hint text ("забрать управление" / "take control"). Key-caps rendered with `fillRoundedRect` + `drawText` at 11px.
- **Action captions:** the server's `begin()` summary format becomes `<tool>|<detail>` (e.g. `click|640,412`, `type|"hello"`, `key|ctrl+s`, `click_until|deny ×12`, `recipe|open-notepad`); the overlay maps tool → localized verb: ru: click Кликаю, move Веду курсор, drag Перетаскиваю, scroll Прокручиваю, type Печатаю, key Нажимаю, window Переключаю окно, find Ищу элемент, wait Жду, screenshot Смотрю, batch Выполняю серию, click_until Перебираю список, recipe Выполняю рецепт, control —; en: Clicking, Moving, Dragging, Scrolling, Typing, Pressing, Switching window, Finding, Waiting, Looking, Running a batch, Grinding a list, Running a recipe. Rendered as `Кликаю · 640,412` (detail muted). Unknown/legacy summaries render as-is.
- **Motion:** show = pill slides from `y = -h` to `y = 14·scale` with ease-out cubic over 220 ms while fading in; hide = fade out 180 ms (alpha multiplier on the whole HUD image); action text change = crossfade 160 ms (render old and new captions with complementary alphas); paused = crossfade to the paused colors, stays 2.5 s, fades. Spark and border breathing share `phase`. Frame loop stays 30 fps; HUD re-render only when something changed or an animation is running (track `dirty`/`animating` so idle costs nothing).
- **Glow shimmer (optional, `border_shimmer`):** while Controlling, a soft brighter segment (Gaussian, σ = 6% of the perimeter, +45% alpha) travels around the frame once per 6 s; implemented in `apply(breath, shimmerPos)` by multiplying each strip's base alpha by `1 + 0.45·exp(−d²/2σ²)` where `d` is the perimeter distance from the pixel's edge position to the shimmer position (precompute each strip pixel's perimeter coordinate once in `renderStrips`). Off when paused.
- **Title from the orchestrator:** `skills/computer-use/SKILL.md`: at the very start of a job call `control{action:"acquire", task:"<caption>"}` with a caption written for the user in their language — verb + object, ≤ 40 characters, no quotes, e.g. `Заполняю дома 5★ жильцами`, `Ищу отчёт в почте`; update it per subtask with `control{action:"hud", task:...}`. `agents/operator.md`: don't override the title unless the orchestrator did not set one.
- **Demo:** `cu demo` shows the sequence: appear → 3 different action captions → paused → hide, so the user can judge every animation; `-o` still proves capture exclusion.

- [ ] **Step 1: Tests first** — `spark_test.go`: `renderSpark` paints the center and a ray tip, leaves the corner transparent, and two phases differ; `hud_test.go`: `captionFor("ru","click|640,412")` → (`Кликаю`, `640,412`), unknown → raw; `renderHUD` with `Alpha: 0.5` halves the max alpha; key-caps present (a pixel inside the first key-cap has the cap background color). `border_test.go`: `apply(1, shimmerPos)` raises alpha near the shimmer position and not far from it.
- [ ] **Step 2: Implement** per the design; keep `platform.Overlay` interface unchanged (`SetTitle`, `SetAction` strings; the format convention lives in server + overlay).
- [ ] **Step 3: Verify visually** with `cu demo -show-in-capture -o …` frames at appear/mid/paused and by viewing them; iterate until it looks like a Claude product (up to three passes); then a real `claude -p` task from outside the repo to see the orchestrator's caption in the HUD.
- [ ] **Step 4: Commit** `feat: Claude-style HUD with animated spark, human captions, motion and glow shimmer`.

---

### Task 23: Recipes that actually get used — server-side suggestions, auto-recorded drafts, success tracking

Measurement (2026-09-08, controller): the same desktop-only Notepad task without recipes = 10 tool calls / 30 s; with a saved recipe present = 15 calls / 47 s and the `recipe` tool was never called (runs counter unchanged). The saved recipe also shows `runs:1, successes:0` (its one replay had a failing step) and nobody repaired it. Conclusion: recipe usage cannot depend on the orchestrator remembering to search; the server must surface recipes at the moment a job starts, record drafts itself, and the prompts must treat a failing replay as something to repair.

**Files:**
- Modify: `internal/server/tools_control.go` (`acquire` with `task` → `suggested_recipes`), `internal/server/tools_recipe.go` (`draft` action; `run` returns failed step details; `save` accepts `auto`), `internal/server/session.go` (trace entries keep the tool + compact args JSON so drafts can be built), `internal/recipes/recipes.go` (`Auto bool`, `LastError string`, `Draft(trace) Recipe` heuristics, search bonus for curated over auto), tests, `skills/computer-use/SKILL.md`, `agents/operator.md`, `README*.md`, `CHANGELOG.md`.

**Design:**
1. **Suggestions at job start.** `control{action:"acquire", task:"…"}` runs `Search(task, foregroundProcess, 3)` and returns `suggested_recipes: [{slug, name, score, params, runs, successes, auto}]` (empty list when none). The tool description says: "acquire returns matching recipes — if one scores ≥ 0.5, run it (`recipe run`) before doing the job by hand."
2. **Trace with arguments.** `traceEntry` gains `Args map[string]any` (the tool's input minus screenshots/regions, values truncated to 200 chars) so a draft can be reconstructed; `recipe trace` returns them.
3. **Drafts.** `recipe{action:"draft", name?, description?}` builds a `Recipe` from the current trace: keeps action tools only (click, drag, scroll, type, key, window, wait, click_until, find), drops screenshots/monitors/windows/pixel/control, collapses consecutive `wait` steps, converts `type` texts longer than 3 words into `{{text1}}`, `{{text2}}` params, inserts `wait{stable:true}` after `window focus`/`key win+…`, sets `App` from the foreground process at acquire time, `Auto: true`. Returns the draft (not saved) so the model can edit and `save` it.
4. **Auto-record.** On `control{action:"release"}` (and on the session's idle release), if the trace has ≥ 4 action steps, no recipe was run in this job, and a task caption was set: save the draft automatically as `Auto: true` with name = caption (slug from it), description = caption + the app name. Auto recipes never overwrite curated ones (same slug → suffix `-2`).
5. **Success tracking and repair.** `run` result lists failed steps (`failed: [{i, tool, error}]`); `Bump` records `LastError`; search ranks: curated (`auto:false`) +0.1, success rate as before, and recipes with `runs ≥ 2 && successes == 0` are dropped from suggestions. Skill: "if `recipe run` reports failed steps, finish the job by hand, then `recipe draft` + `recipe save` with the same name to replace it (`save` on an existing slug updates it and resets counters)".
6. **Prompts.** Skill: mandatory first call `control acquire` with caption; use suggestions ≥ 0.5; at the end `control release` (auto-record happens); after novel jobs prefer `recipe draft` → edit → `save` (curated beats auto). Operator: "if the orchestrator names a recipe, `recipe run` first, then verify with a screenshot and finish any failed step by hand".

- [ ] **Step 1: Tests first** — recipes: `Draft` from a synthetic trace (collapses waits, parametrises long `type`, drops non-action tools); search prefers curated over auto at equal overlap; recipes with runs≥2/successes=0 excluded; `Save` on an existing slug replaces and resets counters. Server (fake harness): `control acquire` with a task returns `suggested_recipes` containing a saved matching recipe with score ≥ 0.5; `control release` after ≥4 actions auto-saves a recipe (`auto:true`) into the store; `recipe run` with a failing step reports `failed` and bumps `successes` unchanged.
