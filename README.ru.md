...
- **Рецепты** — процедурная память: сервер предлагает подходящие рецепты при начале задачи, автоматически записывает черновики из трейса и отслеживает успешность; курированные рецепты ранжируются выше автоматических
...
Плагин предоставляет 20 MCP-инструментов через сервер `desktop`:
...
| `mouse_down` | Нажать и удерживать кнопку мыши (для перетаскивания между окнами) | `x`, `y`, `element`, `button` |
| `mouse_up` | Отпустить удерживаемую кнопку мыши, опционально в x,y | `x`, `y`, `element`, `button` |
| `drag` | Перетащить из одной точки в другую; поддерживает `via` для маршрутных точек | `from`, `to`, `via`, `button`, `duration_ms`, `hold_ms` |
...
| `screenshot_format` | `jpeg` | `CU_SCREENSHOT_FORMAT` | Формат скриншота: `jpeg` (по умолчанию, ~5x быстрее кодирования) или `png` |
| `jpeg_quality` | `90` | `CU_JPEG_QUALITY` | Качество JPEG (1–100) |
...
| `drag_hold_timeout_ms` | `60000` | `CU_DRAG_HOLD_TIMEOUT_MS` | Авто-отпускание удерживаемой кнопки мыши после стольких мс (страховка; минимум 1000) |
| `border_shimmer` | `true` | `CU_BORDER_SHIMMER` | Бегущий яркий блик на светящейся рамке при управлении |
| `recipes_auto_record` | `true` | `CU_RECIPES_AUTO_RECORD` | Автоматическая запись черновика рецепта при завершении управления |
...
**Как это работает:**
1. При `control{action:"acquire", task:"..."}` сервер автоматически ищет подходящие рецепты и возвращает `suggested_recipes` — отдельный поиск не нужен.
2. Если совпадение набирает >= 0.5 баллов, оператор воспроизводит его: `recipe{action:"run", slug:"...", values:{...}}` — и проверяет результат.
3. При `control{action:"release"}` сервер автоматически записывает черновик рецепта, когда в трейсе >= 4 действий, рецепт не запускался и задан заголовок задачи. Авто-запись логируется в stderr; можно отключить через `recipes_auto_record: false`.
4. `recipe{action:"draft"}` строит рецепт из трейса: оставляет только действия, всегда параметризует текст `type` (никакого открытого текста в рецептах), использует `{{secretN}}` для конфиденциальных данных, добавляет шаги ожидания после переключений приложений.
5. Для наилучшего качества: отредактируйте черновик и сохраните через `recipe{action:"save", ...}`.
...
- **Минимальная версия Windows**: Windows 10 версии 1703+ (API DPI-осведомлённости). Нативное исключение из захвата (`SetWindowDisplayAffinity`) требует Windows 10 версии 2004+; на более ранних версиях оверлей скрывается на время каждого скриншота.
- **Модель доверия пайпа**: именованный пайп принимает подключения только от процессов, запущенных от имени того же пользователя (или локальных администраторов).
