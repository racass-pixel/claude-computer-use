...
Плагин предоставляет 16 MCP-инструментов через сервер `desktop`:
...
| `monitors` | Список мониторов с координатами, DPI, курсором | — |
...
| `recipe` | Процедурная память: поиск, запуск, сохранение, трассировка | `action`, `slug`, `values` |
